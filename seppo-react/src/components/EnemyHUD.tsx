...
export default function EnemyHUD({ enemy, playerDef, actionsLeft, inBattle }: Props) {
  ...
  return (
    <>
      ...
      {/* ── DESKTOP ── */}
      <div
        className="hidden sm:flex flex-col items-end gap-[0.6vh] bg-surface-container/80 backdrop-blur-sm p-[1vh_1rem] pixel-border min-w-[280px] transition-opacity duration-300"
        style={{ opacity: enemy ? 1 : 0 }}
      >
        <div className="flex items-center gap-3 flex-row-reverse w-full">
          <div className="bg-surface-container-highest pixel-border flex items-center justify-center p-0.5 border-tertiary/40 border overflow-hidden flex-shrink-0" style={{ width: 'clamp(2.5rem, 6vh, 4rem)', height: 'clamp(2.5rem, 6vh, 4rem)' }}>
            {enemy?.portrait ? (
