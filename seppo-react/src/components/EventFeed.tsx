...
export default function EventFeed({ entries, currentLevel, currentRound }: Props) {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-30 w-[32rem] pointer-events-none">
      <div className="flex flex-col gap-1 items-center">
        {entries.map(e => (
          <div
            key={e.id}
            className={`${logColors[e.cls] || 'text-on-surface-variant'} ${feedBgs[e.cls] || 'bg-surface-container-lowest/80 border-white/10'} border px-4 py-1 font-label text-sm tracking-wide backdrop-blur-sm whitespace-nowrap animate-feed-in`}
          >
            {e.msg}
          </div>
        ))}
